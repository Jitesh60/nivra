...
- App Router with route groups: `(auth)/login` (`/login`, `/login/setup`, `/login/verify`, `/login/recovery-codes`) and `(dashboard)` (`/`, `/users`, `/users/[id]`, `/documents`, `/documents/[id]`, `/waitlist`, `/admins`, `/account`, `/account/password`), plus the `/logout`, `/waitlist/export` and `/documents/[id]/image` route handlers.
...
- **User detail** (`/users/[id]`): profile, Phone/Email/ID badges, documents (linked to review), active device count and an activity trail. The trail includes audit entries about the user's documents. Suspend, ban and reactivate (SUPER_ADMIN, OPS) each need a reason and a confirm click. Suspend and ban revoke every session.
