...
| Financial info · User payment info | No | — | — | — | Card and UPI details go straight to Razorpay's SDK; Nivra never sees them |
| Financial info · Purchase history | Yes | No | Required | App functionality | Bookings and payments |
| Financial info · Other financial info | Yes | Yes (Razorpay) | Optional (lenders) | App functionality | Bank account, IFSC and PAN for payouts, sent to Razorpay; Nivra keeps only the last digits |
...
**Sharing notes.** Razorpay (payments and payouts), Firebase Cloud Messaging (push), MSG91 (SMS codes) and Sentry (crash reports) process data on Nivra's behalf; Play treats service providers as not "sharing", except where the user's data is sent for the provider's own purposes, which none of these do. The payout details sent to Razorpay are marked shared above to be conservative, because Razorpay also uses them for its own KYC.
