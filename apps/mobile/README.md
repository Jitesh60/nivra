...
- **Release signing:** release builds use `android/key.properties` (the Play upload key) when present; `flutter build appbundle --flavor prod` refuses to build without it. CI builds a debug-signed staging release APK to check R8 and the plugins. Steps: [docs/RELEASE.md](../../docs/RELEASE.md).
- **Store kit:** [store/](store/README.md) has the listing text, Data safety and App Store privacy answers, content rating, and screenshots generated by `flutter test test/store --update-goldens --dart-define=STORE_SCREENSHOTS=true`.
